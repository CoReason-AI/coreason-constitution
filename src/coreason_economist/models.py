...
class Budget(BaseModel):
    ...
    """
    financial_limit: Optional[float] = Field(None, description="Max cost in USD per request/session.")
    latency_limit_ms: Optional[int] = Field(None, description="Max latency in milliseconds.")
    token_limit: Optional[int] = Field(None, description="Max total tokens (input + output).")

class Cost(BaseModel):
    """
    Represents the cost of an action (estimated or actual).
    """
    financial_cost: float = Field(0.0, description="Cost in USD.")
...
class Request(BaseModel):
    ...
    model_name: str
    input_text: str
    task_type: str = "generation" # e.g. "generation", "embedding", "tool_call"
...
class EconomicTrace(BaseModel):
    ...
    decision: Decision
    voc_score: Optional[float] = Field(None, description="Value of Computation score.")
